...
def manual_input():
    ...
    builder = SolverBuilder.with_glove(embedding_path)
    solver = builder.build(conf_path, SummedNearestNeighbour, Cosine, "moderate")
...
def random_task():
    ...
    p_path = ["..", "data", "word_embeddings", "paragram", "paragram-ws-thin.txt"]
    FT_PATH = os.path.join(*p_path)
    solverbuilder = SolverBuilder.with_paragram(embedding_path=FT_PATH)
    for _ in range(20):
        ...
        solver = solverbuilder.build(conf_path, MeanIndividualDistance, Cosine, 'moderate')
        solver.threshold = .25
